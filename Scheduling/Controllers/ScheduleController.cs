...
namespace Scheduling.Controllers
{
    ...
    public class ScheduleController : Controller
    {
        ...
        public async Task<IActionResult> GetCalendar(int month = 0, int year = 0)
        {
            ...
            _log.LogInfo($"Loaded calendar for {_helper.DisplayMonthYear(month, year)}");
        ...
        public async Task<IActionResult> AssignShift(int userId, int shiftId, DateTime date)
        {
            ...
            if (existingSchedule != null)
            {
                switch (shiftId)
                {
                    case 0:
                        ...
                        _log.LogInfo($"Removed the shift of {empName} on {dateStr}");
                        break;
                    case 999:
                        existingSchedule.Comment = "cancelled";
                        _log.LogInfo($"Cancelled the shift of {empName} on {dateStr}");
                        _db.Schedules.Update(existingSchedule);
                        break;
                    case 998:
                        existingSchedule.Comment = null;
                        _log.LogInfo($"Uncancelled the shift of {empName} on {dateStr}");
                        _db.Schedules.Update(existingSchedule);
                        break;
                    default:
                        ...
                        _log.LogInfo($"Updated the shift of {empName} on {dateStr} from '{prevShiftName}' to {shiftName}");
                        _db.Schedules.Update(existingSchedule);
                        break;
                }
            }
            else
            {
                ...
                _log.LogInfo($"Assigned {shiftName} to {empName} on {dateStr}");
            }
        ...
        public async Task<IActionResult> AssignTime(int userId, string time, DateTime date, bool isTimeIn)
        {
            ...
            if (schedule == null)
            {
                ...
                _log.LogInfo($"Assigned {timeStr} time {timeType} to {empName} on {dateStr}");
            }
            else
            {
                if (isTimeIn)
                ...
                if (schedule.Time_in == null && schedule.Time_out == null)
                {
                    ...
                    _log.LogInfo($"Removed schedule for {empName} on {dateStr} as both time in and out are blank");
                }
                else
                {
                    _db.Schedules.Update(schedule);
                    _log.LogInfo($"Updated the time {timeType} of {empName} on {dateStr} to {timeStr}");
                }
            }
        ...
        public async Task<IActionResult> UpdateEmployeeOrder(string order, int month, int year)
        {
            ...
            _log.LogInfo($"Updated Employee order for {_helper.DisplayMonthYear(month, year)}, Order:{order}");
        ...
        public async Task<IActionResult> AssignShiftLeader(int month, int year, int userId)
        {
            ...
            _log.LogInfo($"Assigned {await _user.GetFullname(userId)} as shift leader for {_helper.DisplayMonthYear(month, year)}");
        ...
        public async Task<IActionResult> RemoveShiftLeader(int month, int year, int userId)
        {
            ...
            await _token.UpdateTokenAsync(await _user.GetDepartmentId(userId), year, month);
            _log.LogInfo($"Removed {await _user.GetFullname(userId)} from the {_helper.DisplayMonthYear(month, year)} shift leader list");
        ...
        public async Task<IActionResult> SetShiftLeader(int userId, DateTime date)
        {
            ...
            if (existingSchedule != null)
            {
                ...
                _log.LogInfo($"Assigned {empName} as shift '{existingSchedule.Shift?.Shift_name}' shiftleader for {dateStr}");
                _db.Schedules.Update(existingSchedule);
            }
            else
            {
                ...
                _log.LogInfo($"Assigned {empName} as shiftleader for {dateStr}");
            }
        ...
        public async Task<IActionResult> UnsetShiftLeader(int userId, DateTime date)
        {
            ...
            if (existingSchedule != null)
            {
                ...
                _log.LogInfo($"Removed {empName} as shift '{existingSchedule.Shift?.Shift_name}' shiftleader for {dateStr}");
                _db.Schedules.Update(existingSchedule);
            }
            else
            {
                ...
                _log.LogInfo($"Removed {empName} as shiftleader for {dateStr}");
            }
